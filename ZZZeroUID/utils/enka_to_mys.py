...
def render_weapon_detail(weapon: Dict, current_level: int, star_tier: str):
    ...
    base_value = (
        ...
        + base_value
        * (
            weapon['level'][str(current_level)]['Rate']
            + weapon['stars'][star_tier]['StarRate']
        )
        / 10000
    )
    ...
    if rand_value:
        rand_value = rand_value + rand_value * (
            weapon['stars'][star_tier]['RandRate'] / 10000
        )
...
def _calculate_base_stat(
    ...
    if level > 10 and extra_key:
        base_value += (
            extra_level[act]['Extra'].get(extra_key, {}).get('Value', 0)
        )
...
def _get_value_str(
    value: float, prop_level: int, prop_id: str, is_main_r: bool = False
):
    if is_main_r:
    ...
    for char in enka_data['PlayerInfo']['ShowcaseDetail']['AvatarList']:
        ...
        for relic in char['EquippedList']:
            ...
            for main_prop in _equip['MainPropertyList']:
                value = main_prop['PropertyValue'] + MAIN_PROP_VALUE[
                    str(main_prop['PropertyId'])
            ...
            weapon['properties'] = [
                {
                    ...
                    'base': (
                        f"{round(rand)/100}%"
                        if _weapon['rand_props_id'] in PERCENT_ID
                        else str(rand)
                    ),
                }
            ]
        ...
        weapon['main_properties'] = [
            {
                ...
                'base': (
                    f"{round(base)/100}%"
                    if _weapon['props_id'] in PERCENT_ID
                    else str(base)
                ),
            }
        ]
        weapon['talent_title'] = _weapon['talents'][str(weapon['star'])][
            'Name'
        ]
        weapon['talent_content'] = _weapon['talents'][str(weapon['star'])][
            'Desc'
        ]
        ...
        props['Attack'] += (
            props['AttackBase']
            + (props['AttackAdd'] / 10000) * props['Attack']
        )
        props['Defence'] += (
            props['DefenceBase']
